...
def get_conda_list(conda_list_file=None):
    ...
    if conda_list_file is None:
        ...
        result = subprocess.run(
            ["conda", "list"], stdout=subprocess.PIPE, text=True, check=True
        )
    ...
    if match:
        ...
        if base_name in package_versions:
            ...
            if not pin:
                ...
                return (
                    package_line,
                    f"Mismatch (existing pin: {pin}, current: {version})",
                )
            else:
...
def update_yaml_file(yaml_file, package_versions):
    ...
    for idx, line in enumerate(lines):
        ...
        if stripped_line.startswith("packages:") or stripped_line.startswith(
            "dependencies:"
        ):
        ...
        if in_packages_section and (
            "- pip" in stripped_line or stripped_line == "pip:"
        ):
            in_pip_section = True
            updated_lines.append(line)
            continue
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Add minimum pins to unpinned packages in a YAML file."
    )
    parser.add_argument(
        "yaml_file",
        help="Path to the YAML file (anaconda-project.yml or environment.yml).",
    )
    parser.add_argument(
        "--conda-list", help="Path to a saved 'conda list' output file."
    )
